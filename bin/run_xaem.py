...
def index_ref(step=1):
    trascript = config.get('xaem','transcript_fa') if config.get('xaem','transcript_fa') else f'{bindir}/../ref/GRCh38/transcript.fa.gz'
...
def count_matrix(step=3):
    x_matrix = config.get('xaem','x_matrix') if config.get('xaem','x_matrix') else f'{bindir}/../ref/GRCh38/X_matrix.RData'
